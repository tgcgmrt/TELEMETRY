...
//	int	i, j;
	...
	pthread_mutex_unlock(&eh.eMutex);
                      ...
                      pcBufRead = (char*)malloc(256);
                         // pcBufRead = (char*)realloc(pcBufRead, dwRxSize);
	  		 memset(pcBufRead, 0x00, 256);
       ...
       /*      dwRxSize = 0;			
		while ((dwRxSize < 3 ) && (ftStatus == FT_OK)) 
                  ...
                  if(ftStatus == FT_OK) 
                    {  
                        ftStatus = FT_Read(ftHandle, pcBufRead,dwRxSize, &dwBytesInQueue);
                           ...
                           {
                                            printf("FT_Read = %d\n", dwBytesInQueue);
			        	     printf(" Adress byte pcBufWrite[0] = 0x%02X\n", j, pcBufRead[0]);
                                             printf(" Packet  byte pcBufWrite[1] = 0x%02X\n", j, pcBufRead[1]);
...
// Manually read first 2 bytes..1st byte gives U me the address byte and second byte will give me packet length...
       /*        dwRxSize = 0;			
		while ((dwRxSize < 10 ) && (ftStatus == FT_OK)) 
          ...
          */        
            /* ftStatus =  FT_GetQueueStatus(ftHandle, &dwRxSize);
            fprintf(stderr," Number of bytes in GetQueuestatus is %d",dwRxSize);
	         if(ftStatus == FT_OK) 
                 {
