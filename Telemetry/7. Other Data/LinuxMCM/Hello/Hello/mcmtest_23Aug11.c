   ...
   buff[0]=*buf_ptr0;
	...
	/*	while ((dwRxSize < BUF_SIZE ) && (ftStatus == FT_OK)) 
			...
			ftStatus = FT_GetQueueStatus(ftHandle, &dwRxSize);
                   ...
                   /*     dwRxSize=0;
             FT_GetQueueStatus(ftHandle, &dwRxSize);
             fprintf(stderr," Number of bytes in first GetQueuestatus is %d\n ",dwRxSize);
             FT_Read(ftHandle, pcBufRead,dwRxSize, &dwBytesInQueue);
            ...
            if(ftStatus == FT_OK)
             {
              fprintf(stderr," Number of bytes in second  GetQueuestatus is %d\n", dwRxSize);
             FT_Read(ftHandle, pcBufRead,dwRxSize, &dwBytesInQueue);
              printf("pcBufWrite[%d] = 0x%02X\n", j, pcBufRead[1]);
              } */
              //memset(pcBufRead, 0x00, 100);
	    ...
	    //     if(ftStatus == FT_OK) 
              //   {
                      ...
                      //   buffer == (pcBufRead+1);
                       ...
                       // sprintf(Nobytes," Number of bytes to be read is %d",pcBufRead[1]);
