...
void Set_LORef3()
{
 ...
 printf("\nunsigned lor3 = %d", ref_lo3);

    LOR_Arr1[0] = 'R';
    LOR_Arr1[1] = (char)ref_lo3 *2;
    LOR_Arr1[2] = 0;
    LOR_Arr1[3] = 0;
    LOR_Arr1[4] = 0;
    LOR_Arr1[5] = 0;

 printf("\nAfter converting to hex : %x",LOR_Arr1[1]);
...
void Set_LO3()
{
 char LO_Arr1[10], rem[10], Temp_Arr[10];
 int padding, cntr_1, i;
 unsigned long lo;
 ...
 //sprintf(LO3_Mon,"%ld %s",lo_frq,"MHz");
 ...
 i=0;
  ...
  strcpy(LO_Arr1 + padding, Temp_Arr);

  LO_Arr1[0] = 'K';
   printf("\n(LO_Arr) : %s\n",LO_Arr1);

 for(cntr_1=0; cntr_1 < strlen(LO_Arr1); cntr_1++)
  {
    if((LO_Arr1[cntr_1] > 96) && (LO_Arr1[cntr_1] < 123))
    LO_Arr1[cntr_1] = LO_Arr1[cntr_1] - 32;
  }

 strcpy(Temp_Arr, LO_Arr1);

 printf("\nSending over SPI (LO_Arr1) : %s\n",LO_Arr1);
