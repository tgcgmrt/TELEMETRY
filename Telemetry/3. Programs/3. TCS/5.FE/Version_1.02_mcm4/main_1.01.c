      ...
      if(!_f_strcmp(Wrpr_Cmd.cmd_elem.subsysid,SYSTEM_2))
       {
         ...
         if(!_f_strcmp(Wrpr_Cmd.cmd_elem.cmd_name,Cmd_200))
          ...
          {
           return 10;
          } else if(!_f_strcmp(Wrpr_Cmd.cmd_elem.cmd_name,Cmd_230) || !_f_strcmp(Wrpr_Cmd.cmd_elem.cmd_name,Cmd_231))
          {
           return 10;
          }
         else
...
//  printf("\nEntering Command_Process_FE");
  ...
  */
  ...
  else if(!_f_strcmp(Wrpr_Cmd.cmd_elem.cmd_name,Cmd_230))
  {
    fecbSet.cmdCode = 30;

    num_mask_32 =  Wrpr_Cmd.cmd_data.numpkt;
    for (j = 0 ; j < num_mask_32;j++)
    {  _f_strcpy(tmpstr,Wrpr_Cmd.cmd_data.prmvalue[j]);
        printf("value %s \n", tmpstr);

      mask_32[j] = _f_strtol(tmpstr,NULL,16);
      printf(" mask %ld \n", mask_32[j]);
      printf(" mask [%d] %x \n", j, mask_32[j]);
  ...
  { for (i = 0 ; i < TSET; i++ ) {
     ...
     if(!strcasecmp(Wrpr_Cmd.cmd_data.prmname[j], fevar[i]))
     {
        switch(i)
        {
             ...
             case FE_NG_FREQ     :  fecbSet.ngFreq = atoi(Wrpr_Cmd.cmd_data.prmvalue[j]);
                                    if( fecbSet.ngFreq > 3 || fecbSet.ngFreq  < 0 )
                                    fecbSet.ngFreq = 0;
                                     break;

             case FE_WALSH_PAT   :  fecbSet.walshPat = atoi(Wrpr_Cmd.cmd_data.prmvalue[j]);
                                    if( fecbSet.walshPat > 7 || fecbSet.walshPat  < 0 )
                                     fecbSet.walshPat = 0;
                                     break;

             case FE_WALSH_FREQ  :  fecbSet.walshFreq = atoi(Wrpr_Cmd.cmd_data.prmvalue[j]);
