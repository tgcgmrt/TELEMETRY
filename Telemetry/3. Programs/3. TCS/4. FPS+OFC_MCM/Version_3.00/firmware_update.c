...
struct {
	char	running[20];		// information on the currently running firmware
	char	temp[20];			// information on the uploaded file
	int	show_temp;			// =0 if no temp. firmware available otherwise =1
   int	in_status;        // installation status
   int   up_status;        // upload status
	char	error[30];        // contains error message if any
