...
cofunc int OFC_mcm_clnt()
{
   ...
   if(sock_established(&OFC_Socket))
   {
    ...
    do{
       ...
       read_bytes1 = 0;
       yield;
       //printf("\n wait counter in recv OFC %d \n", wait_var1);
      }while(tcp_tick(&OFC_Socket)&& (wait_var1++ < 200000) );    // This loop continuously listens over socket for incoming command //
   }
   http_handler();
...
cofunc int FPS_mcm_clnt()
{
   ...
   if(sock_established(&FPS_Socket))
   {
    ...
    do{
       ...
       read_bytes = 0;
       yield;
             // printf("\n wait counter in recv FPS %d \n", wait_var);
      }while(tcp_tick(&FPS_Socket) && (wait_var++ < 200000) );    // This loop continuously listens over socket for incoming command //
   }
   http_handler();
